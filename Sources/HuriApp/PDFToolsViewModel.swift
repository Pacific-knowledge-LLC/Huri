...
final class PDFToolsViewModel: ObservableObject {
    ...
    func importPDFs(_ urls: [URL]) {
        ...
        guard !pdfs.isEmpty else {
            operation = .failure("Sélectionnez au moins un document PDF.")
            return
        }
    ...
    func exportMerged() {
        guard !pages.isEmpty, let destination = savePDFPanel(title: "Exporter le PDF fusionné") else {
            return
        }
        export(plan: PDFEditorPlan(pages: pages), to: destination, verb: "Fusion")
    }

    func exportSelection() {
        let selection = selectedPages
        guard !selection.isEmpty,
              let destination = savePDFPanel(title: "Exporter les pages sélectionnées")
        else { return }
        export(plan: PDFEditorPlan(pages: selection), to: destination, verb: "Extraction")
    }

    func splitSelectedDocument() {
        guard let sourceURL = selectedSourceURL else { return }
        let panel = NSOpenPanel()
        panel.title = "Choisir le dossier pour les PDF découpés"
        panel.prompt = "Découper ici"
        ...
        operation = .working("Découpe de \(sourceURL.lastPathComponent)…")
        let editor = services.pdfEditor
        operationTask = Task { [weak self] in
            do {
                ...
                self?.operation = .success(
                    message: "\(urls.count) document\(urls.count > 1 ? "s" : "") créé\(urls.count > 1 ? "s" : "")",
                    urls: urls
                )
    ...
    func revealOperationResults() {
        guard case let .success(_, urls) = operation, !urls.isEmpty else { return }
    ...
    private func export(plan: PDFEditorPlan, to destination: URL, verb: String) {
        operation = .working("\(verb) en cours…")
        let editor = services.pdfEditor
        operationTask = Task { [weak self] in
            do {
                try await editor.export(plan: plan, to: destination)
                guard !Task.isCancelled else { return }
                self?.operation = .success(message: "\(verb) terminée", urls: [destination])
            } catch is CancellationError {
                self?.operation = .idle
            } catch {
                self?.operation = .failure(error.localizedDescription)
...
private enum PDFRangeError: LocalizedError {
    ...
    var errorDescription: String? {
        "Les plages doivent ressembler à « 1-3, 5, 8-10 »."
    }
}
