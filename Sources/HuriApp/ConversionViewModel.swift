...
final class ConversionViewModel: ObservableObject {
  ...
  func importFiles(_ urls: [URL]) {
    ...
    Task {
      ...
      importWarning =
        failures.isEmpty
        ? nil
        : HuriL10n.text("import.failure.header") + failures.joined(separator: "\n")
      isImporting = false
  ...
  func chooseDestination() {
    ...
    panel.title = HuriL10n.text("conversion.destination.title")
    panel.prompt = HuriL10n.text("conversion.destination.choose")
  ...
  func revealResult() {
    guard case .success(let result) = state else { return }
...
private enum ImportPayload: @unchecked Sendable {
  ...
  var index: Int {
    switch self {
    case .success(let index, _, _), .failure(let index, _, _):
      index
    }
  }
}
