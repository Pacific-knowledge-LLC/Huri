...
public actor LocalConversionCoordinator: ConversionExecuting {
    ...
    public func convert(
        ...
        guard !plan.assets.isEmpty else {
            throw ConversionError.invalidPlan("Ajoutez au moins un fichier à convertir.")
        }
        guard plan.destinationDirectory.isFileURL else {
            throw ConversionError.invalidPlan("Choisissez un dossier de destination local.")
        }
        guard plan.outputFormat != .unknown else {
            throw ConversionError.invalidPlan("Choisissez un format de sortie valide.")
        }
        if plan.options.removeBackground, plan.outputFormat != .png {
            throw ConversionError.invalidPlan(
                "Le détourage intelligent est disponible pour les sorties PNG."
            )
        }
        ...
        for asset in plan.assets {
            guard capabilities.supports(
                ...
                throw ConversionError.unsupported(
                    "La conversion \(asset.format.displayName) → \(plan.outputFormat.displayName) n’est pas disponible."
                )
            }
        }
        ...
        for (index, asset) in plan.assets.enumerated() {
            ...
            progress(
                ConversionProgress(
                    ...
                    totalUnitCount: plan.assets.count,
                    message: "Conversion de \(asset.filename)…"
                )
            )
            ...
            if converted.count > 1 {
                warnings.append(
                    "« \(asset.filename) » a produit \(converted.count) fichiers, un par page."
                )
            }
            progress(
                ConversionProgress(
                    ...
                    totalUnitCount: plan.assets.count,
                    message: "\(asset.filename) terminé"
                )
            )
            await Task.yield()
        }
    ...
    private func convert(
        ...
        case .image:
            if outputFormat == .pdf {
                ...
                return [try await pdfEngine.imageToPDF(
            ...
            if options.removeBackground {
                return [try await backgroundRemovalEngine.removeBackground(
                    from: asset.sourceURL,
                    to: destination
                )]
            }
            return [try imageEngine.convert(
                source: asset.sourceURL,
                to: outputFormat,
                destination: destination,
                options: options
        ...
        case .audio, .video:
            ...
            return [try await mediaEngine.convert(
                source: asset.sourceURL,
        ...
        case .unsupported:
            throw ConversionError.unsupported(
                "Le type de « \(asset.filename) » n’est pas reconnu."
            )
        }
    }
    ...
    private func renderPDFToImages(
        ...
        if options.removeBackground {
            renderDirectory = temporaryDirectory
