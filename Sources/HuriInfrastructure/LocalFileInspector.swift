...
public final class LocalFileInspector: FileTypeDetecting, @unchecked Sendable {
    ...
    public func inspect(url: URL) async throws -> FileAsset {
        guard url.isFileURL else {
            throw ConversionError.unreadable("Huri ne peut inspecter que des fichiers locaux.")
        }
        guard FileManager.default.isReadableFile(atPath: url.path) else {
            throw ConversionError.unreadable(
                "Le fichier « \(url.lastPathComponent) » est introuvable ou illisible."
            )
        }
        ...
        let resolved = magicFormat ?? typeFormat ?? (
            extensionFormat == .unknown ? nil : extensionFormat
        ) ?? .unknown

        let warning: String?
        if let magicFormat, extensionFormat != .unknown, magicFormat != extensionFormat {
            warning = "Le contenu est du \(magicFormat.displayName), malgré l’extension .\(url.pathExtension)."
        } else {
            warning = nil
        }
    ...
    private func readHeader(at url: URL) throws -> Data {
        do {
            ...
            throw ConversionError.unreadable(
                "Impossible de lire « \(url.lastPathComponent) » : \(error.localizedDescription)"
            )
        }
    }
