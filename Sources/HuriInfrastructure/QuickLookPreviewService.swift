    ...
    public func previewData(
        ...
        do {
            ...
            throw ConversionError.conversionFailed(
                "Quick Look ne peut pas prévisualiser « \(asset.filename) » : \(error.localizedDescription)"
            )
        }
        ...
        guard let destination = CGImageDestinationCreateWithData(
            ...
            throw ConversionError.conversionFailed("Impossible de créer l’aperçu PNG.")
        }
        CGImageDestinationAddImage(destination, representation.cgImage, nil)
        guard CGImageDestinationFinalize(destination) else {
            throw ConversionError.conversionFailed("Impossible de finaliser l’aperçu PNG.")
        }
        return data as Data
    }
}
