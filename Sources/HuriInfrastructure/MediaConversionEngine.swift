...
struct MediaConversionEngine: Sendable {
    func convert(
        ...
        guard let session = AVAssetExportSession(asset: asset, presetName: preset) else {
            throw ConversionError.unsupported(
                "AVFoundation ne peut pas préparer cette conversion média."
            )
        }
        guard session.supportedFileTypes.contains(outputType) else {
            throw ConversionError.unsupported(
                "Cette combinaison média n’est pas prise en charge par AVFoundation sur ce Mac."
            )
        }
        ...
        guard session.status == .completed else {
            ...
            throw ConversionError.conversionFailed(
                session.error?.localizedDescription ?? "La conversion média a échoué."
            )
        }
        do {
            ...
            throw ConversionError.conversionFailed(
                "Impossible de finaliser « \(destination.lastPathComponent) » : \(error.localizedDescription)"
            )
        }
        return destination
    }
    ...
    private func convertPCM(
        ...
        do {
            ...
            throw ConversionError.conversionFailed(
                "Le transcodage audio PCM a échoué : \(error.localizedDescription)"
            )
        }
        do {
            try FileManager.default.moveItem(at: temporaryURL, to: destination)
    ...
    private func writePCM(
        ...
        guard let buffer = AVAudioPCMBuffer(
            ...
            throw ConversionError.conversionFailed("Impossible d’allouer le tampon audio.")
        }
    ...
    private func fileType(for format: FileFormat) throws -> AVFileType {
        ...
        default:
            throw ConversionError.unsupported(
                "Le format média \(format.displayName) n’est pas disponible."
            )
        }
    }
}
